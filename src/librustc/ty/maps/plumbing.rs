...
pub(super) struct QueryMap<'tcx, D: QueryDescription<'tcx>> {
    ...
    pub(super) map: FxHashMap<D::Key, QueryResult<QueryValue<D::Value>>>,
}
...
/*
                ...
                loop {
                    let job = if let Some(value) = tcx.maps.$name.borrow().map.get(&key) {
                        match *value {
                            QueryResult::Started(ref job) => { job.clone(); panic!("started job") },
                            QueryResult::Complete(ref value) => {
                                profq_msg!(tcx, ProfileQueriesMsg::CacheHit);
                                tcx.dep_graph.read_index(value.index);
                                return Ok((&value.value).clone());
...
*/
                ...
                if dep_node.kind.is_anon() {
                    ...
                    let res = tcx.cycle_check(span, Query::$name(key), || {
                        let job = Self::start_job(tcx, key);
            ...
            fn start_job(_tcx: TyCtxt<'a, $tcx, 'lcx>, _key: $K) -> Lrc<QueryJob> {
                let job = Lrc::new(QueryJob::new());
                                                  ...
                                                  -> Result<$V, CycleError<'a, $tcx>>
            {
                ...
                let (result, job) = if let Some(result) = result {
                    (result, Lrc::new(QueryJob::new()))
                } else {
                    // We could not load a result from the on-disk cache, so
                    // recompute.
                    let ((result, _), job) = tcx.cycle_check(span, Query::$name(key), || {
                        let job = Self::start_job(tcx, key);
