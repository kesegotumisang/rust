...
pub(super) struct CycleError<'tcx> {
    pub(super) span: Span,
    pub(super) cycle: Vec<(Span, Query<'tcx>)>,
}
                    ...
                    let job = if let Some(value) = tcx.maps.$name.borrow().map.get(&key) {
                        match *value {
                            QueryResult::Started(ref job) => { job.clone();
                                ...
                                if tcx.query().tls {
                                    println!("trying to start a tls query");
                                }
                                println!("({}) started job {:?} latch {:x}",
                                QUERY_DEPTH.load(Ordering::SeqCst),
                                    key, &job.latch as *const _ as usize);
                                for i in tcx.query().stack.iter() {
                                    println!("    query stack entry {:?}", i);

                                }
                                panic!("already started query");}
                            QueryResult::Complete(ref value) => {
                                profq_msg!(tcx, ProfileQueriesMsg::CacheHit);
                                tcx.dep_graph.read_index(value.index);
                                return Ok((&value.value).clone());
...
*/
                ...
                if dep_node.kind.is_anon() {
                    ...
                    let res = Self::start_job(tcx, span, key, |tcx| {
            ...
            fn start_job<F, R>(tcx: TyCtxt<'a, $tcx, 'lcx>,
                               span: Span,
                               key: $K,
                               compute: F)
                                                  ...
                                                  -> Result<$V, CycleError<$tcx>>
            {
                ...
                let (result, job) = if let Some(result) = result {
                    ...
                    let ((result, _), job) = Self::start_job(tcx, span, key, |tcx| {
